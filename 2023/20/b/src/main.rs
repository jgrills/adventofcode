...
fn main() {
    ...
    number_of_nodes += 1;
    node[number_of_nodes] = Node{name:"rx", kind:'R', target_text:"", target:[TARGETS; TARGETS], targets:0, on:false, sources:0, source:[0;SOURCES], last_low:[true;SOURCES] };
    number_of_nodes += 1;
    ...
    'rx: loop {
        ...
        while !deque.is_empty() {
            ...
            match result {
                ...
                None => {
                    // println!("process {}=node {} {} {} X", input.node, low_text(input.low), node[ni].name, node[ni].kind);
                }
            };
        }
