...
fn find_prev(slice: &[i32]) -> i32 {
    ...
    let down = &arr[0..sz-1];
    let mut next = 0;
    if again { next = find_prev(down); };
    let result = slice.first().unwrap() - next;
    println!("  result {} {}", result, next);
    ...
    if let Ok(lines) = read_lines(file_path) {
        for line in lines {
            if let Ok(contents) = line {
                ...
                let next = find_prev(&arr[0..count]);
                let answer = arr[0] - next;
