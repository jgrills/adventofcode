...
int64_t atoi(std::string_view text)
{
    int64_t out;
...
{
    int64_t out;
    const std::from_chars_result result = std::from_chars(line.data(), line.data() + line.size(), out);
    assert(result.ec == std::errc());
    line.remove_prefix(result.ptr - line.data());
    return out;
}
